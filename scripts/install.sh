...
RELEASES_URL="https://github.com/foolin/sumdiff/releases"
FILE_BASENAME="sumdiff"
LATEST="$(curl -sf https://goreleaser.com/static/latest)"

test -z "$VERSION" && VERSION="$LATEST"

test -z "$VERSION" && {
	echo "Unable to get sumdiff version." >&2
	exit 1
}
...
(
	...
	if command -v cosign >/dev/null 2>&1; then
		...
		cosign verify-blob \
			--certificate-identity-regexp "https://github.com/foolin/sumdiff.*/.github/workflows/.*.yml@refs/tags/$VERSION" \
